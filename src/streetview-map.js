...
let mapType = "roadmap";  // "roadmap"(地図) | "hybrid"(空撮=衛星＋ラベル)。パネルのトグルで切替
...
export function initStreetViewMap(maps, host, { center, zoom = 16 } = {}) {
  ...
  gmap = new maps.Map(host, {
    ...
    zoom,
    mapTypeId: mapType,        // 地図/空撮はパネルヘッダの自前トグル(setStreetViewMapType)で切替
    // ズーム等の標準UI・Google のロゴ・著作権/帰属は残す（消さない）。
    mapTypeControl: false,     // 標準の地図/航空コントロールは小パネルでは出さず、自前トグルで操作
...
// 地図 ⇄ 空撮(航空写真) 切替。type は "roadmap" | "hybrid"。次回 init でも保持する。
export function setStreetViewMapType(type) {
  mapType = type === "hybrid" ? "hybrid" : "roadmap";
  if (gmap) gmap.setMapTypeId(mapType);
  return mapType;
}
export function getStreetViewMapType() { return mapType; }
